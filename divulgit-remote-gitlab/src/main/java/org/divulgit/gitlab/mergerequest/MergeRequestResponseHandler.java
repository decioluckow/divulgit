...
@Slf4j
@Component
public class MergeRequestResponseHandler {
	
    public List<GitLabMergeRequest> handle200ResponseMultipleResult(ResponseEntity<String> response) throws RemoteException {
        try {
    ...
    public GitLabMergeRequest handle200ResponseSingleResult(ResponseEntity<String> response) throws RemoteException {
        try {
            Jackson2ObjectMapperBuilder builder = new Jackson2ObjectMapperBuilder();
            ObjectMapper objectMapper = builder.build();
            return objectMapper.readValue(response.getBody(), GitLabMergeRequest.class);
        } catch (JsonProcessingException e) {
            throw parseException(response, e);
        }
    }
