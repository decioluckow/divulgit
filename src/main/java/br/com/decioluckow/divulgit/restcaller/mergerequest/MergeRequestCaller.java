...
public class MergeRequestCaller {
    ...
    public List<GitLabMergeRequest> retrieveMergeRequests(final Origin origin, final String token) throws CallerException {
        final List<GitLabMergeRequest> mergeRequests = new ArrayList<>();
        retrieveMergeRequests(token, mergeRequests, "1");
        return mergeRequests;
    }

    private void retrieveMergeRequests(final String token, final List<GitLabMergeRequest> mergeRequests, final String page) throws CallerException {
        ResponseEntity<String> response = restCaller.call("https://git.neogrid.com/api/v4/projects/{0}/merge_requests?iids[]=??????per_page=100&page=" + page, token);
        if (response.getStatusCode().is2xxSuccessful()) {
            mergeRequests.addAll(handle200Response(response));
        ...
        if (!Strings.isNullOrEmpty(nextPage)) {
            retrieveMergeRequests(token, mergeRequests, nextPage);
        }
    }

    private List<GitLabMergeRequest> handle200Response(ResponseEntity<String> response) throws CallerException {
        try {
    ...
    private void handleErrorResponse(ResponseEntity<String> response) throws CallerException {
        try {
