package br.com.decioluckow.divulgit.tasks.project;
...
public class ProjectScanTask extends Task {
    ...
    public ProjectScanTask(Origin origin, User user, String token) {
        this.origin = origin;
        this.user = user;
        this.token = token;
    }
    ...
    public TaskUniqueKey uniqueKey() {
        return new TaskUniqueKey("origin:" + origin.getId() + "|user:" + user.getId());
    }

    @Override
    public void execute() {
        try {
            log.info("Starting scanning projects for origin {}", origin.getId());
